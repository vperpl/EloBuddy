        ...
        public static List<Vector2> PathFind(Vector2 start, Vector2 end)
        {
            ...
            try
            {
                ...
                foreach (var pol in outerPolygons)
                {
                    for (var i = 0; i < pol.Points.Count; i++)
                    {
                        ...
                        var node = nodes.FirstOrDefault(node1 => node1.Point == pol.Points[i]) ??
                                   new Node(pol.Points[i]);

                        nodes.Add(node);
                        var i1 = i;
                        foreach (var nodeToAdd in outerPolygons.SelectMany(polygon => (from point in polygon.Points
                            where Utils.CanReach(pol.Points[i1], point, innerPolygons, true)
                            select nodes.FirstOrDefault(node1 => node1.Point == point) ??
                                           new Node(point))))
                        {
                            nodes.Add(nodeToAdd);
                ...
                foreach (var node in nodes)
                {
                    ...
                    EloBuddy.Drawing.DrawCircle(node.Point.To3D(), 10, System.Drawing.Color.Aqua);
