...
namespace Evade
{
    ...
    public struct FoundIntersection
    {
        ...
        public FoundIntersection(float distance, int time, Vector2 point, Vector2 comingFrom)
        {
            ...
            Valid = (Math.Abs(point.X) > 0.00001) && (Math.Abs(point.Y) > 0.00001);
    ...
    public class Skillshot
    {
        ...
        public bool Evade()
        {
            ...
            if (!Config.SkillShotsMenu["IsDangerous" + SpellData.MenuItemName].Cast<CheckBox>().CurrentValue && Config.Menu["OnlyDangerous"].Cast<KeyBind>().CurrentValue)
            {
                _cachedValue = false;
            ...
            _cachedValue = Config.SkillShotsMenu["Enabled" + SpellData.MenuItemName].Cast<CheckBox>().CurrentValue;
            _cachedValueTick = Utils.TickCount;
        ...
        public void Game_OnGameUpdate()
        {
            ...
            if (SpellData.CollisionObjects.Count() > 0 && SpellData.CollisionObjects != null &&
                Utils.TickCount - _lastCollisionCalc > 50 && Config.CollisionMenu["EnableCollision"].Cast<CheckBox>().CurrentValue)
            {
        ...
        public Vector2 GlobalGetMissilePosition(int time)
        {
            ...
            t = (int) Math.Max(0, Math.Min(End.Distance(Start), t * SpellData.MissileSpeed / 1000d));
