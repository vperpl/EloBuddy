...
namespace Evade
{
    public static class Evader
    {
            ...
            bool onlyGood = false)
        {
            ...
            foreach (var skillshot in Program.DetectedSkillshots.Where(skillshot => skillshot.Evade()))
            {
            ...
            bool dontCheckForSafety = false)
        {
            ...
            foreach (var targetType in validTargets)
            {
                switch (targetType)
                {
                    case SpellValidTargets.AllyChampions:
                        ...
                        allTargets.AddRange(ObjectManager.Get<AIHeroClient>().Where(ally => ally.IsValidTarget(range) && !ally.IsMe && ally.IsAlly));
                        break;


                    case SpellValidTargets.AllyMinions:
                        allTargets.AddRange(
                            EntityManager.MinionsAndMonsters.CombinedAttackable.Where(x=>(x.Distance(Player.Instance) < range) && (x.Team == Player.Instance.Team)));
                        break;

                    case SpellValidTargets.AllyWards:

                        allTargets.AddRange(ObjectManager.Get<Obj_AI_Minion>().Where(gameObject => gameObject.Name.ToLower().Contains("ward") && gameObject.IsValidTarget(range) && (gameObject.Team == ObjectManager.Player.Team)));
                        break;

                    case SpellValidTargets.EnemyChampions:
                        allTargets.AddRange(ObjectManager.Get<AIHeroClient>().Where(enemy => enemy.IsValidTarget(range, true)));
                    ...
                    case SpellValidTargets.EnemyWards:
                        allTargets.AddRange(ObjectManager.Get<Obj_AI_Minion>().Where(gameObject => gameObject.Name.ToLower().Contains("ward") && gameObject.IsValidTarget(range, true)));
                        break;
                    default:
                        return null;
                }
            ...
            foreach (var target in allTargets.Where(target => dontCheckForSafety || Program.IsSafe(target.ServerPosition.To2D()).IsSafeB))
            {
                if (isBlink)
                {
